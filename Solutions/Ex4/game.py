...
"""
explication of my strategy:
In the check sequence function, we returned -1 or 1 if 0 people win. 
Now, we calculate in the quadruplet how many "O" and how many "X" are.
We give more importance if there is one, two or three "sum_human" because the people will be more near to the wend of the game.
So, we return sum_human*2 with a negative value, like in the first version where we return -1.
If no people win, this is this value that we will return and this value that we will return in the "value" function.
And, this value will pass in the abmax and abmin function.
So, we influence directly with this line of codes the strategy to win the HUMAN. 
"""
...
def getNext(s):
    ...
    ns = []
    # we over all of the possibilities ans we return a list of them.
